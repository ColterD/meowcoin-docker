...
async function authPlugin(fastify) {
    ...
    fastify.decorate('authorize', (roles) => {
        return async (request, reply) => {
            let userRole;
            // TODO: Replace 'any' with a proper type for request.user
            if (request.user && typeof request.user === 'object' && 'role' in request.user) {
...
async function registerPlugins(fastify) {
    ...
    fastify.decorate('config', config_1.config);
    fastify.decorate('axios', require('axios').default.create());
